...
public class Analytics_AUTHENTICATIONLEVEL_SUBTITLE {
	static int newrow = 0;
	static int newrow_orion = 0;
	static boolean flag_present = false;

	public static void Analytics_AUTHENTICATIONLEVEL_SUB() throws Exception {
		...
		int cafe_AuthenticationLevel = 0, cafe_statustitle = 0;
		if (row_count > 0 || column_count > 0) {
			...
			for (int i = 0; i <= row_count; i++) {
				...
				if (cellvalue != null) {
					...
					data = "";
				cellvalue = row.getCell(cafe_statustitle);
				if (cellvalue != null) {
					if (cellvalue.getCellType() == Cell.CELL_TYPE_NUMERIC)
				...
				if (data.equalsIgnoreCase("C")) {
					if (!data_ntid.contains("Full with CPNI")) {
						row_num_analytics = sheet_analytics.createRow(j);
				...
				if (data.equalsIgnoreCase("X")) {
					if (!data_ntid.contains("Partial with CPNI")) {
						row_num_analytics = sheet_analytics.createRow(j);
						int noOfColumns = sheet.getRow(0).getLastCellNum();
						for (int k = 0; k < noOfColumns; k++) {
							cellvalue = row.getCell(k);
		...
		File new_name = new File(".\\Test_Result\\Analytics_AUTHENTICATIONLEVEL_SUBTITLE_"
