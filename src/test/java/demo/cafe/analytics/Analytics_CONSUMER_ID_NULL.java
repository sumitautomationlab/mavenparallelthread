...
public class Analytics_CONSUMER_ID_NULL {
	static int newrow = 0;
	static int newrow_orion = 0;
	static boolean flag_present = false;

	public static void Analytics_CONSUMER_ID() throws Exception {
		...
		if (row_count > 0 || column_count > 0) {
			...
			for (int i = 1; i <= row_count; i++) {
				...
				row = sheet.getRow(i);
				Cell cellvalue = row.getCell(cafe_ConsumerID);
				if (cellvalue != null) {
					if (cellvalue.getCellType() == Cell.CELL_TYPE_NUMERIC)
						data = NumberToTextConverter.toText(cellvalue.getNumericCellValue());
		...
		File new_name = new File(
				".\\Test_Result\\Analytics_CONSUMER_ID_NULL_" + Analytics_Test_Cases.file_name + ".xlsx");
		if (!flag_present) {
			if (old_name.exists()) {
				if (old_name.delete()) {
