...
public class Analytics_ITGNameField_null {
	static int newrow = 0;
	static int newrow_orion = 0;
	static boolean flag_present = false;

	public static void Analytics_ITGNameField_null_truck() throws Exception {
		...
		if (row_count > 0 || column_count > 0) {
			...
			for (int i = 0; i <= row_count; i++) {
				...
				row = sheet.getRow(i);
				Cell cellvalue = row.getCell(cafe_ITGName);
				if (cellvalue != null) {
					if (cellvalue.getCellType() == Cell.CELL_TYPE_NUMERIC)
						data = NumberToTextConverter.toText(cellvalue.getNumericCellValue());
		...
		File new_name = new File(
				".\\Test_Result\\Analytics_ITGNameField_null_" + Analytics_Test_Cases.file_name + ".xlsx");
		if (!flag_present) {
			if (old_name.exists()) {
				if (old_name.delete()) {
