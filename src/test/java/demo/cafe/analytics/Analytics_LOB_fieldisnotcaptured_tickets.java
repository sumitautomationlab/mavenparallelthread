...
public class Analytics_LOB_fieldisnotcaptured_tickets {
	static int newrow = 0;
	static int newrow_orion = 0;
	static boolean flag_present = false;

	public static void Analytics_LOBfieldisnotcaptured_tickets() throws Exception {
		...
		for (int i = 0; i < wb.getNumberOfSheets(); i++) {
			...
			if ((sheet.getSheetName().toString()).equals("CAFENX_TruckRollDetails")) {
				flag = i;
		...
		int cafe_TicketNumber = 0, cafe_LOB = 0;
		if (row_count > 0 || column_count > 0) {
			...
			for (int i = 0; i <= row_count; i++) {
				...
				if (cellvalue != null) {
					...
					data = "";
				cellvalue = row.getCell(cafe_LOB);
				if (cellvalue != null) {
					if (cellvalue.getCellType() == Cell.CELL_TYPE_NUMERIC)
						data_lob = NumberToTextConverter.toText(cellvalue.getNumericCellValue());
					else if (cellvalue.getCellType() == Cell.CELL_TYPE_BOOLEAN)
						data_lob = Boolean.toString((cellvalue.getBooleanCellValue()));
					else
						data_lob = cellvalue.getStringCellValue();
				} else
					data_lob = "";
				// System.out.println("value is :" + data);
				if (!data.contains("NULL")) {
					if (data_lob.contains("NULL")) {
						row_num_analytics = sheet_analytics.createRow(j);
		...
		File new_name = new File(".\\Test_Result\\Analytics_LOB_fieldisnotcaptured_tickets_"
