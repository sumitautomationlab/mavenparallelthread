...
public class Analytics_CAFELOCATION_CAFEROLE_NULL {
	static int newrow = 0;
	static int newrow_orion = 0;
	static boolean flag_present = false;

	public static void Analytics_CAFELOCATION_CAFEROLE() throws Exception {
		...
		for (int i = 0; i < wb.getNumberOfSheets(); i++) {
			...
			if ((sheet.getSheetName().toString()).equals("CAFENX_AGENTDETAILS_FACT_Report")) {
				flag = i;
		...
		int cafe_location = 0, cafe_role = 0;
		if (row_count > 0 || column_count > 0) {
			for (int i = 0; i <= 0; i++) {
				...
				for (int k = 0; k < noOfColumnsfirst; k++) {
					...
					if (data.contains("CAFELOCATION"))
						cafe_location = k;
					if (data.contains("CAFEROLE"))
						cafe_role = k;
			...
			for (int i = 0; i <= row_count; i++) {
				...
				row = sheet.getRow(i);
				Cell cellvalue = row.getCell(cafe_location);
				if (cellvalue != null) {
					if (cellvalue.getCellType() == Cell.CELL_TYPE_NUMERIC)
						data = NumberToTextConverter.toText(cellvalue.getNumericCellValue());
				...
				cellvalue = row.getCell(cafe_role);
				if (cellvalue != null) {
					if (cellvalue.getCellType() == Cell.CELL_TYPE_NUMERIC)
		...
		File new_name = new File(".\\Test_Result\\Analytics_CAFELOCATION_CAFEROLE_NULL_"
