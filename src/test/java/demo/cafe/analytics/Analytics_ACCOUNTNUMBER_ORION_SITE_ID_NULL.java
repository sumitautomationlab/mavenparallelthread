...
public class Analytics_ACCOUNTNUMBER_ORION_SITE_ID_NULL {
	static int newrow = 0;
	static int newrow_orion = 0;
	static boolean flag_present = false;

	public static void Analytics_ACCOUNTNUMBER_ORION_SITE_ID() throws Exception {
		...
		for (int i = 0; i < wb.getNumberOfSheets(); i++) {
			...
			if ((sheet.getSheetName().toString()).equals("CAFENX_CustomerDim")) {
				flag = i;
		...
		int cafe_accountnumber = 0, cafe_Orion_siteID = 0;
		if (row_count > 0 || column_count > 0) {
			for (int i = 0; i <= 0; i++) {
				...
				for (int k = 0; k < noOfColumnsfirst; k++) {
					...
					if (data.contains("AccountNumber"))
						cafe_accountnumber = k;
					if (data.contains("Orion_siteID"))
						cafe_Orion_siteID = k;
			...
			for (int i = 0; i <= row_count; i++) {
				...
				row = sheet.getRow(i);
				Cell cellvalue = row.getCell(cafe_accountnumber);
				if (cellvalue != null) {
					if (cellvalue.getCellType() == Cell.CELL_TYPE_NUMERIC)
						data = NumberToTextConverter.toText(cellvalue.getNumericCellValue());
					...
					data = "";
				cellvalue = row.getCell(cafe_Orion_siteID);
				if (cellvalue != null) {
					if (cellvalue.getCellType() == Cell.CELL_TYPE_NUMERIC)
		...
		File new_name = new File(".\\Test_Result\\Analytics_ACCOUNTNUMBER_ORION_SITE_ID_NULL_"
